...
**Mission** (1문장): 개인 PostgreSQL DB + 리포트 팩트 + WebSearch 기반 KRX 종목 스크리닝·분석 자동화로, 사용자에게 투자 판단용 메인/와일드카드 리스트와 Risk Flag 기반 Signal을 제공한다.
...
**왜 지금**: 2026년 별도재무 DB 구축 완료(연결구분 컬럼)로 연결/별도 FCF 구분이 가능해짐. 상법개정(자사주 소각 의무화)으로 저PBR/주주환원 분석 수요 증가. stocks-db MCP 연동으로 Claude Code에서 DB 직접 쿼리 가능.

**타겟 사용자**: PostgreSQL DB(metainfo)를 직접 운영하는 한국 개인투자자. R 스크립트(InsertCorpData.R)로 재무 데이터를 수집하고, Claude Code에서 종목 분석/스크리닝을 수행. 투자 스타일: 중장기 펀더멘탈 + 배당/주주환원.

**사용자 행동 모델:**
- **분석 빈도**: 월 1~2회 정기 스크리닝 + 실적 발표/이벤트 시 수시 개별 종목 분석
- **워크플로우**: R 스크립트로 DB 갱신 → Claude Code에서 `/stock-analyzer` 호출 → 리포트 검토 → 매수/보유/매도 판단
- **기술 수준**: SQL 직접 작성 가능, R 숙련, Python 기초. Claude Code 프롬프트로 분석 지시.
...
**전제 조건:**
- `stocks-db` MCP 서버가 user scope로 설치되어 있어야 함 (`claude mcp list`로 확인)
...
### 2. Web Search (Claude 내장 WebSearch 도구)
API 키 불필요. Claude Code의 내장 WebSearch 도구를 사용하여 실시간 뉴스 수집.

**전제 조건:**
- WebSearch 도구가 활성화되어 있어야 함 (기본 활성)
- 인터넷 연결 필요
...
| Step 2b | 리포트 팩트 추출 (리포트 PDF/컨센서스/WebSearch) + Risk Flag 산출 | 종목별 `{valuation_basis, investment_thesis, risks, moat, risk_flag, risk_flag_reasons[], risk_flag_sources[]}` |
| Step 2c | 별도재무 확인 (연결 부채비율 > 150% 종목 대상) | 해당 종목 `sep_DEBT` override |
| Step 2d | 종합 판정 + 6개 리스트 산출 + Signal/Confidence | 최종 Markdown 리포트 내용 |
| Step 2e | Obsidian vault 저장 | `{YYYY-MM-DD}_스크리닝_리포트.md` 파일 |
...
### Step 2: 스킬 (정성 — Claude 수행)

| 단계 | 내용 | 대상 |
|------|------|------|
| 테마 태깅 | 밸류업(고정) + 동적 테마(직전 3개월 기반) 해당 여부 | 330개 전체 |
| 커버리지 분리 | broker_count 기준 Covered/Uncovered pool 분리 | 330개 → Covered + Uncovered |
| 리포트 팩트 | Covered 메인 3개 TOP 10 합집합(~25개): Obsidian PDF / Uncovered 3개 TOP 5 합집합(~10개): WebSearch | 상위 ~35개 |
...
| Step 2e Obsidian 저장 | `리포트 저장 완료 → ~/Documents/Obsidian Vault/StockAnalysis/screening/{YYYY-MM-DD}_스크리닝_리포트.md` (동일 경로 존재 시 `_v2`, `_v3` 접미사 부여) |
...
### Step 2 상세: 스킬 (정성 분석 — Claude 수행)

Step 2는 R 스크립트 결과(~330개)를 받아 정성 분석 후 6개 리스트(메인 가치/성장/혼합 TOP 10 + 와일드카드 가치/성장/혼합 TOP 5)를 산출한다.

#### 실행 패턴 (순차/병렬)

**스크리닝 경로의 Step 2 내부 반복 실행 지점은 순차가 기본**. 병렬 허용 범위는 아래 표의 "병렬 허용" 행에 한정 (2a 독립 WebSearch 3쿼리 + 개별 종목 비교 요청의 종목별 MCP 재무 쿼리).

| 지점 | 방식 | 근거 |
|------|------|------|
| 2a 동적 테마 WebSearch 3쿼리 (시장/ETF/정책) | 병렬 허용 | 3개 쿼리는 독립 탐색이며, 결과 통합 후 "3건 이상 독립 소스" 기준으로 채택 |
| 2a Phase 2 테마별 ETF·산업분류 매핑 | 순차 | 선정 테마별 매핑 결과를 DB 산업분류·매출 비중 확인에 누적 적용 |
| 2b Covered PDF Read (~25건) | 순차 | Read 실패 시 즉시 `risk_flag=Unknown`+"리포트 없음" 처리 후 다음 종목 진행 |
| 2b Uncovered WebSearch (~10건) | 순차 | rate limit 방지 + 검색 품질 보장 |
...
**연속 실패 중단 조건:**
- 2b PDF Read **5종목 연속 실패** → 남은 Covered 종목 `risk_flag=Unknown` 일괄 처리. Done 판정 "부분 성공"으로 하향. Confidence 하향 요인 "PDF Read 대량 실패(-1)" 적용.
- 2b PDF **전체 실패율 50% 이상** → Obsidian vault 접근 불가 경고 표시. PDF Read 실패한 Covered 종목에 한해 WebSearch fallback(Uncovered 경로)으로 전환하여 사업 모델·최근 실적·리스크만 수집 (성공한 PDF의 추출 결과는 유지).
- 2a Phase 2 ETF/산업분류 매핑에서 **테마별 종목 0건이 3개 테마 연속** → 해당 테마 라벨 부착 생략 (남은 테마만 진행, 경고 메시지).

**Agent 사용 여부**: 이 스킬은 sub-agent를 스폰하지 않는다 (Claude 단일 주체가 MCP + Read + WebSearch + Bash로 선형 수행). Agent 스폰이 필요한 확장 시나리오가 생기면 이 섹션 하단에 스폰 패턴을 추가한다.
...
**동적 테마 발견 (WebSearch):**
실행일 기준 직전 3개월간의 시장 테마를 WebSearch로 탐색:
...
**Phase 2: 테마별 ETF/종목 매핑 (WebSearch)**
선정된 각 테마에 대해:
1. `"{테마명} ETF 구성종목 {YYYY}"` 검색 → 해당 테마 ETF가 있으면 편입 종목 파악
2. DB `산업분류` 매칭 — 테마와 관련된 산업분류 식별
3. 산업분류 미매칭 종목은 WebSearch로 매출 비중 확인하여 50% 이상이면 해당으로 판정
...
Covered pool의 가치/성장/혼합 각 TOP 10 합집합(약 20~25개)에 대해 애널리스트 리포트 PDF에서 구조화된 팩트를 추출한다. 와일드카드(Uncovered)는 PDF가 없으므로 WebSearch로 사업 모델·최근 실적·리스크를 별도로 확인하여 1~2줄 요약을 병기한다.

**리포트 PDF는 `qualitative_pipeline.py`로 Obsidian vault에 사전 수집한다.** 스크리닝 실행 시 Claude가 상위 종목의 최신 리포트 PDF를 Read tool로 직접 읽고 아래 스키마로 구조화하여 리스트에 병기한다. **정규식 기반 자동 추출은 사용하지 않는다** (이전 `report_extractor.py` 참조는 제거됨).
...
| `risk_flag_sources[]` | 출처 URL | string list | `risk_flag = Red` 시 필수, 리포트/DART/WebSearch URL |
...
2. Read tool로 PDF를 읽는다 (Claude는 PDF 읽기 가능)
...
**Step 2c에서 스킬의 역할** (Claude 수행): R 산출 결과를 리포트에 병기하기만 한다:
...
**플레이스홀더 치환 규칙**: `stocks-db` MCP는 raw SQL을 받으므로 `:year_list`와 `:top_codes`는 **MCP 호출 직전에 Claude가 SQL 문자열에 리터럴로 치환**한다 (바인딩 파라미터 미지원). 양쪽 모두 작은따옴표 포함 CSV 형태 튜플. 치환 실패 시 MCP 호출 보류.
...
이 섹션은 **Signal 스코어링 입력이 아니다**. 리포트 팩트의 "투자논리·리스크·해자" 항목과 위 "Risk Flag 산출" 절차의 Red 조건(단일제품 70%, 경영권 분쟁, 배당 제약, 감사의견 한정, 유상증자+적자, 특허소송, 관리종목)을 뒷받침할 근거 URL을 찾는 WebSearch 쿼리 가이드다.
...
| 와일드 | **와일드카드-가치 TOP 5** | Uncovered | `score_value` 내림 | 5 | 산업분류, sv, sg, ROE(별도), PER, PBR, DIV, 시총, 주요 테마, WebSearch 1줄 |
| 와일드 | **와일드카드-성장 TOP 5** | Uncovered | `score_growth` 내림 | 5 | 산업분류, sv, sg, ROE(별도), PER, CAGR, OPM변화, 시총, 주요 테마, WebSearch 1줄 |
| 와일드 | **와일드카드-혼합 TOP 5** | Uncovered | 가치+성장 합집합 중 `avg=(sv+sg)/2` 내림 | 5 | 산업분류, sv, sg, avg, ROE(별도), PER, PBR, **CV%, Core/Sat**, 시총, 주요 테마, WebSearch 1줄 |
...
- 와일드카드 정성 정보: 애널 리포트 부재이므로 **WebSearch로 사업 모델·최근 실적·리스크 1~2줄 확인하여 병기**. Obsidian vault PDF가 있으면 참고 가능
...
| 4 | **와일드카드-가치 TOP 5** | Uncovered · sv 상위 + WebSearch 1줄 + ⚠️ 유의 |
| 5 | **와일드카드-성장 TOP 5** | Uncovered · sg 상위 + WebSearch 1줄 + ⚠️ 유의 |
| 6 | **와일드카드-혼합 TOP 5** | Uncovered · avg 상위 + WebSearch 1줄 + ⚠️ 유의 |
...
### 원칙
1. **Dim A는 R 스크립트가 산출** — `score_value`/`score_growth`는 R 스크립트가 계산한 최종값을 사용하며, 스킬(Claude)은 재계산하지 않는다. R 스크립트 내부에서는 아래 보정이 이미 적용된 값이므로 스킬 단계에서 추가 조정할 필요가 없다.
...
**출처 필수**: 각 Red Flag는 리포트 본문 또는 DART 공시·WebSearch URL을 동반해야 한다. 출처 없는 추정 = Red Flag 인정 안 함(None으로 분류).
...
**팩트 추출**: Claude Read tool로 Obsidian vault의 PDF 직접 읽기 (정규식 기반 자동 추출 미사용 — "2b. 리포트 팩트 추출" 섹션 참조)
...
4. Step 2b: 리포트 팩트 추출 (canonical 7필드 스키마, "2b. 리포트 팩트 추출" 섹션 참조)
   - Covered 메인 3개 합집합(~25개): Obsidian PDF 읽기 (Claude Read tool)
   - Uncovered 와일드카드 3개 합집합(~10개): WebSearch 1~2줄
...
6. Step 2d: 종합 판정 + 6개 리스트 산출 + Signal/Confidence (메인 3 × 와일드카드 3 대칭)
   ...
   - 와일드카드 공통: + WebSearch 1줄 + ⚠️ 커버리지 부재 유의
...
7. Step 2e: Obsidian vault 저장 → `~/Documents/Obsidian Vault/StockAnalysis/screening/{YYYY-MM-DD}_스크리닝_리포트.md`
...
**각 리스트의 종목에 병기되는 리포트 팩트** (메인 3개 섹션=가치/성장/혼합 TOP 10은 Obsidian PDF Read, 와일드카드 3개 섹션=가치/성장/혼합 TOP 5는 WebSearch 1~2줄):
- 밸류에이션 근거 (방법론 + 목표가)
- 투자논리 (핵심 카탈리스트 1~3개, 구체적 수치)
- 리스크 (구체적 목록)
- 해자/경쟁우위

**최종 출력물은 Obsidian vault에 마크다운 파일로 저장한다:**
`~/Documents/Obsidian Vault/StockAnalysis/screening/{YYYY-MM-DD}_스크리닝_리포트.md`
...
2. **웹 검색 결과는 출처 URL 필수** — 뉴스/전망/컨센서스 데이터는 반드시 WebSearch 또는 WebFetch로 확인하고, 결과에 출처 URL을 포함. URL 없는 뉴스 인용 = 날조로 간주
...
| 시나리오 | 대응 |
|----------|------|
| stocks-db MCP 서버 미연결 | 사용자에게 `claude mcp list`로 확인 안내. 사용자가 종목을 직접 지정한 경우 WebSearch만으로 제한적 분석 가능 (Confidence 최대 Low, 재무 데이터 없음 명시). 스크리닝 요청이면 MCP 필수 안내 후 중단 |
...
| 시나리오 | 대응 |
|----------|------|
| `ModuleNotFoundError: requests/bs4` | 사용자에게 `pip install requests beautifulsoup4` 안내 + 정성 데이터 수집 스킵 → WebSearch fallback (Confidence 1단계 하향) |
...
3. 하향 요인: 뉴스 0건(-1), 뉴스 90일+ 경과(-1), 신규 상장 3년 미만(-1), DB 불가 웹전용(-2), 테마 태깅 미실행(-1), PDF Read 대량 실패(-1) — "실행 패턴" 섹션의 연속 실패 중단 조건 참조
...
1. **데이터 원칙**: stocks-db(재무제표) + WebSearch(뉴스)로 분석 가능한 항목만 포함. 실시간 시세, 가격 시계열, 주문 실행이 필요한 분석은 제외.
...
- **데이터 소스**: stocks-db PostgreSQL (재무제표) + R 스크립트 (정량 스크리닝) + WebSearch (뉴스/감성) + **korea-stock-analyzer MCP (기술적 지표·수급·DCF, 선택적)**
...
| "반도체 산업 전망 분석" | **포함** (종목 없이도) | WebSearch 기반 정성 분석 (원칙 1) |
