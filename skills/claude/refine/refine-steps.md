...
- **역할 경계**: `claude-scorer`는 독립 점수, `cross-reviewer`는 근거 조정, Sonnet Workflow proposer는 수정안 텍스트만 소유한다. main만 acceptance/rollback/ledger closure를 확정하며, 역할 실패를 다른 역할의 무근거 승인으로 대체하지 않는다.
...
Agent({ name: "claude-scorer", subagent_type: "general-purpose", model: "sonnet",   // 모델 정책: 채점 실무 계층
  prompt: "너는 /refine의 채점자이다.
    ...
    4b. code/test/integrate 모드면 인지복잡도도 검증하라: main path를 가리는 3단+ 중첩, 긴 if/switch 체인, 반복 부정, callback chain, boolean flag/mode string 분기, 한 함수의 parsing/I/O/mutation/formatting 혼재, 불필요한 mutable state, dead/future-only branch.
...
기준: (a) 주장이 코드로 뒷받침되나 (b) 빠진 항목 (c) 모호 표현 반례 (d) 인지복잡도 — main path를 가리는 중첩/긴 조건 체인/boolean-mode 분기/혼재 책임/불필요 state (e) 날조 수치=0점.
...
- code/test/integrate에서 main path가 deep nesting, callback chain, 반복 부정, 긴 조건 체인, boolean flag/mode string으로 만든 divergent behavior, parsing/I/O/mutation/formatting 혼재, 불필요한 mutable state, dead/future-only branch에 묻히면 인지복잡도 약점으로 포함한다.
...
채택본 항목별 정확성/완전성/부작용 검증은 이 단계에서 하지 않는다 — APPLY 5번 codex-reviewer가 diff 교차 리뷰와 함께 통합 수행한다.

3. PROPOSE_COMPARE 채택본(또는 조건 미충족 시 claude 단독 초안)을 그대로 APPLY로 넘긴다 — 항목별 검증은 APPLY 5번 codex-reviewer가 통합 수행한다.
...
4. code/test/integrate: 단순화 pass를 수행한다. main path가 먼저 읽히도록 guard clause/early return으로 중첩을 줄이고, parsing·branching·I/O·mutation·formatting이 섞인 함수는 도메인 규칙 기준으로 분리하며, dead/duplicate branch와 불필요한 mutable state를 제거하고, boolean flag/mode string이 divergent behavior를 만들면 이름 있는 함수 또는 더 단순한 dispatch shape를 검토한다. 새 추상화는 실제 분기/상태 복잡도를 줄일 때만 유지한다.
5. Codex 교차 리뷰 — codex 가용 시 필수 호출 (**채택 수정안 항목별 정확성/완전성/부작용 검증**(PROPOSE에서 분리 검증하던 임무를 통합) **+ 전체 diff 회귀 리뷰**를 한 번에 수행. PROPOSE의 비교 채택은 이 검증을 대체하지 않는다 — 병합·재작성본은 여기서 처음 adversarial하게 검토된다. 미설치/spawn 실패/timeout 시에는 아래 정의된 예외 분기를 따른다):
```
...
- mutation sanity와의 관계: red의 "구현 전 실패 확인"이 신규 테스트의 mutation check를 대체한다. 기존 테스트를 강화한 항목은 기존 mutation 규칙(APPLY 7번)을 그대로 따른다.
