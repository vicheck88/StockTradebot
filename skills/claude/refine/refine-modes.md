...
| Simplicity | 20% | 명시적 인지복잡도 pass에서 main path가 edge case보다 먼저 읽히는가? 불필요한 추상화/계층/wrapper 없는가? premature abstraction? 동일 동작을 더 적은 코드로? dead code · 미사용 import · future-only 분기/feature flag 회피? 가드 클로즈로 중첩 ≤2단, 긴 if/switch 체인·반복 부정 회피, boolean flag/mode string으로 한 함수에 여러 동작을 넣지 않기, 결합 조건식은 이름 붙여 추출, 파라미터 ≤4, 추상화는 실제 분기/상태 복잡도를 줄일 때만 유지? |
...
- 인지 복잡도 게이트: 가드 클로즈로 대체 가능한 3단+ 중첩, 긴 if/switch 체인, 반복 부정, callback chain, 또는 실사용 1곳뿐이며 분기/상태 복잡도를 줄이지 않는 헬퍼/래퍼가 잔존하면 `Simplicity` 상한 70
- 함수 책임 게이트: 한 함수가 parsing, branching, I/O, mutation, formatting을 함께 들고 있고 도메인 규칙 기준으로 분리 가능하면 `Structure` 상한 75 + `Simplicity` 상한 70
- mode/flag 게이트: boolean flag 또는 mode string이 한 함수 안에서 여러 divergent behavior를 만들면, 별도 이름의 함수나 더 단순한 dispatch shape를 검토하기 전까지 `Simplicity` 상한 70
- TDD ON 라운드(refine-steps.md `### TDD 변형`)에서 red 선행 없이 구현된 항목 1건+ → `Test Coverage` 상한 70 (ROUND LEDGER의 TDD 행으로 검증)
