...
FETCH_CONSENSUS <- TRUE       # FnGuide 컨센서스 조회 (대형주만, ~5분 소요)
...
raw <- tryCatch(
  as.data.table(dbGetQuery(conn, sprintf("
    SELECT 종목코드, 종목명, 시장구분, 산업분류, 시가총액, \"현재가(종가)\",
      관리여부, 자본, 부채, 매출액, 영업이익,
      지배주주순이익, 당기순이익, 배당수익률,
...
# ============================================================
# 7c. 별도재무 비교 및 왜곡 감지
# 연결 vs 별도 괴리를 파악하여 지주회사 왜곡, 금융자회사 왜곡 감지
# ============================================================
sep_data <- tryCatch(
  as.data.table(dbGetQuery(conn, sprintf("
    SELECT f.종목코드,
      MAX(CASE WHEN f.계정='당기순이익' THEN f.값 END) as sep_ni,
      MAX(CASE WHEN f.계정='자본' THEN f.값 END) as sep_equity,
      MAX(CASE WHEN f.계정='부채' THEN f.값 END) as sep_debt,
      MAX(CASE WHEN f.계정='매출액' THEN f.값 END) as sep_revenue,
      MAX(CASE WHEN f.계정='영업이익' THEN f.값 END) as sep_oi
    FROM metainfo.연간재무제표 f
    WHERE f.연결구분 = '별도'
      AND f.일자 = (SELECT MAX(일자) FROM metainfo.연간재무제표 WHERE 연결구분='별도')
      AND f.종목코드 IN (%s)
      AND f.종류 IN ('포괄손익계산서','재무상태표')
    GROUP BY f.종목코드
  ", all_codes_sql))),
  error = function(e) {
    log_step("경고", "별도재무 조회 실패: %s", conditionMessage(e))
    data.table(종목코드 = character())
  })

if (nrow(sep_data) > 0) {
  filtered <- merge(filtered, sep_data, by = "종목코드", all.x = TRUE)

  # 별도 ROE, 별도 부채비율
  filtered[, `:=`(
    sep_ROE  = fifelse(!is.na(sep_equity) & sep_equity > 0 & !is.na(sep_ni),
                       sep_ni / sep_equity * 100, NA_real_),
    sep_DEBT = fifelse(!is.na(sep_equity) & sep_equity > 0 & !is.na(sep_debt),
                       sep_debt / sep_equity * 100, NA_real_)
  )]

  # 지주왜곡 플래그: 별도 적자 + 연결 흑자
  filtered[, holding_distortion := fifelse(
    !is.na(sep_ni) & sep_ni < 0 & !is.na(지배주주순이익) & 지배주주순이익 > 0,
    TRUE, FALSE)]

  # 부채비율 override: 연결 >150% & 별도 <150% → 별도 기준이 실질
  filtered[, debt_override := fifelse(
    !is.na(DEBT) & DEBT > 150 & !is.na(sep_DEBT) & sep_DEBT < 150,
    TRUE, FALSE)]

  # 연결-별도 순이익 괴리율 (높을수록 자회사 의존도 높음)
  filtered[, consol_gap := fifelse(
    !is.na(지배주주순이익) & 지배주주순이익 > 0 & !is.na(sep_ni),
    round((지배주주순이익 - sep_ni) / 지배주주순이익 * 100, 1),
    NA_real_)]

  n_holding <- sum(filtered$holding_distortion, na.rm = TRUE)
  n_debt_ov <- sum(filtered$debt_override, na.rm = TRUE)
  log_step("별도재무", "별도 데이터 %d종목 확보. 지주왜곡 %d개, 부채비율 override %d개",
           nrow(sep_data), n_holding, n_debt_ov)

  # score_value 보정은 섹션 8 이후로 이동 (score_value가 계산된 뒤 적용)
} else {
  filtered[, `:=`(sep_ROE = NA_real_, sep_DEBT = NA_real_,
                  holding_distortion = FALSE, debt_override = FALSE,
                  consol_gap = NA_real_, sep_ni = NA_real_,
                  sep_equity = NA_real_, sep_debt = NA_real_,
                  sep_revenue = NA_real_, sep_oi = NA_real_)]
  log_step("별도재무", "별도 데이터 없음, 스킵")
}
...
# 8b. 별도재무 기반 score_value 보정
if ("holding_distortion" %in% names(filtered)) {
  # 지주왜곡: 별도 적자인데 연결 ROE가 높은 종목 → score_value 5점 감점
  n_adj <- sum(filtered$holding_distortion, na.rm = TRUE)
  filtered[holding_distortion == TRUE, score_value := pmax(score_value - 5, 0)]
  # 부채비율 override: 연결 >150% & 별도 <150% → score_value 1점 가산
  n_debt <- sum(filtered$debt_override, na.rm = TRUE)
  filtered[debt_override == TRUE, score_value := pmin(score_value + 1, 10)]
  if (n_adj > 0 || n_debt > 0)
    log_step("보정", "지주왜곡 %d종목 감점, 부채override %d종목 가점", n_adj, n_debt)
}
...
if (FETCH_CONSENSUS) {
  ...
  if (consensus_sourced) {
    ...
    log_step("컨센서스", "FnGuide 조회 시작 (%d종목)...", length(large_codes))

    consensus_list <- list()
    for (i in seq_along(large_codes)) {
      code <- large_codes[i]
      tryCatch({
        ...
        if (!is.null(r)) {
          ...
          # 분기 컨센서스에서 forward 4분기 영업이익/순이익 합산
          fwd_oi  <- NA_real_
          fwd_ni  <- NA_real_
          fwd_eps <- NA_real_
          if (!is.null(r$quarterly) && nrow(r$quarterly) > 0) {
            q <- r$quarterly
            # 잠정(P) + 추정(E) 분기만 사용
            fwd_q <- q[q$type %in% c("P", "E"), ]
            if (nrow(fwd_q) >= 2) {
              fwd_oi  <- sum(fwd_q$op_income, na.rm = TRUE)
              fwd_ni  <- sum(fwd_q$net_income, na.rm = TRUE)
              fwd_eps <- sum(fwd_q$eps, na.rm = TRUE)
              # 4분기 미만이면 연환산 (비례 확대)
              n_fwd <- nrow(fwd_q)
              if (n_fwd < 4) {
                fwd_oi  <- fwd_oi / n_fwd * 4
                fwd_ni  <- fwd_ni / n_fwd * 4
                fwd_eps <- fwd_eps / n_fwd * 4
              }
            }
          }
          # 연간 컨센서스 EPS가 있으면 우선 사용 (더 정확)
          annual_eps <- if (!is.null(r$eps) && nrow(r$eps) >= 1) r$eps$value[1] else NA_real_
          best_eps <- if (!is.na(annual_eps)) annual_eps else fwd_eps

          consensus_list[[code]] <- data.table(
            종목코드      = code,
            target_avg   = if (!is.null(tp$avg)) tp$avg else NA_real_,
            target_max   = if (!is.null(tp$max)) tp$max else NA_real_,
            target_min   = if (!is.null(tp$min)) tp$min else NA_real_,
            broker_count = if (!is.null(tp$broker_count)) as.integer(tp$broker_count) else 0L,
            opinion_buy  = if (!is.null(op$buy)) as.integer(op$buy) else 0L,
            opinion_hold = if (!is.null(op$hold)) as.integer(op$hold) else 0L,
            opinion_sell = if (!is.null(op$sell)) as.integer(op$sell) else 0L,
            opinion_avg  = if (!is.null(op$avg_score)) op$avg_score else NA_real_,
            eps_fwd1     = if (!is.null(r$eps) && nrow(r$eps) >= 1) r$eps$value[1] else NA_real_,
            eps_fwd2     = if (!is.null(r$eps) && nrow(r$eps) >= 2) r$eps$value[2] else NA_real_,
            fwd_oi       = fwd_oi,
            fwd_ni       = fwd_ni,
            fwd_eps      = best_eps
          )
      ...
      if (i %% 50 == 0) log_step("컨센서스", "  %d/%d 완료", i, length(large_codes))
    }

    if (length(consensus_list) > 0) {
      consensus_dt <- rbindlist(consensus_list)
      result <- merge(result, consensus_dt, by = "종목코드", all.x = TRUE)

      # 매수 비율 (0~10 스케일)
      result[, buy_ratio := fifelse(
        !is.na(opinion_buy),
        round(opinion_buy / pmax(opinion_buy + opinion_hold + opinion_sell, 1) * 10, 1),
        NA_real_)]

      # 목표가 괴리율 (%)
      result[, target_gap := fifelse(
        !is.na(target_avg) & !is.na(`현재가(종가)`) & `현재가(종가)` > 0,
        round((target_avg - `현재가(종가)`) / `현재가(종가)` * 100, 1),
        NA_real_)]

      # Forward PER = 현재가 / 컨센서스 EPS
      result[, fwd_PER := fifelse(
        !is.na(fwd_eps) & fwd_eps > 0 & !is.na(`현재가(종가)`),
        round(`현재가(종가)` / fwd_eps, 1),
        NA_real_)]

      # Forward ROE = 컨센서스 EPS × 발행주식수 / 자본 × 100
      # 발행주식수 = 시총 / 현재가
      result[, fwd_ROE := fifelse(
        !is.na(fwd_eps) & fwd_eps > 0 & !is.na(자본) & 자본 > 0 &
        !is.na(`현재가(종가)`) & `현재가(종가)` > 0,
        round(fwd_eps * (시가총액 / `현재가(종가)`) / 자본 * 100, 1),
        NA_real_)]

      # Blended EPS: 경과 월수 기반 FY1+FY2 가중평균
      current_month <- as.integer(format(Sys.Date(), "%m"))
      fy_remaining <- 12 - current_month + 1  # 올해 남은 개월 (12월 결산 기준)
      result[, blended_eps := fifelse(
        !is.na(eps_fwd1) & !is.na(eps_fwd2),
        round(eps_fwd1 * fy_remaining / 12 + eps_fwd2 * (12 - fy_remaining) / 12, 1),
        eps_fwd1)]

      # Blended PER
      result[, blended_PER := fifelse(
        !is.na(blended_eps) & blended_eps > 0 & !is.na(`현재가(종가)`),
        round(`현재가(종가)` / blended_eps, 1),
        NA_real_)]

      # Forward OPM = 컨센서스 영업이익 / 매출
      result[, fwd_OPM := fifelse(
        !is.na(eps_fwd1) & !is.na(fwd_oi) & fwd_oi > 0,
        {
          # 연간 컨센서스 매출이 있으면 사용
          fwd_rev_idx <- match(종목코드, consensus_dt$종목코드)
          NA_real_  # fwd_OPM은 Grid1에서 직접 가져와야 정확 — 여기서는 skip
        },
        NA_real_)]

      # ============================================================
      # Forward 지표로 score_value 보정 (커버 >= 10사만)
      # PER, ROE를 forward로 교체 후 score_value 재계산
      # ============================================================
      FWD_MIN_BROKERS <- 10

      fwd_eligible <- result[!is.na(broker_count) & broker_count >= FWD_MIN_BROKERS &
                             !is.na(blended_PER) & blended_PER > 0]
      n_fwd_eligible <- nrow(fwd_eligible)

      if (n_fwd_eligible > 0) {
        # Forward 적용 대상: PER과 ROE를 blended 값으로 교체
        result[!is.na(broker_count) & broker_count >= FWD_MIN_BROKERS &
               !is.na(blended_PER) & blended_PER > 0,
               `:=`(PER = blended_PER,
                    ROE = fifelse(!is.na(fwd_ROE) & fwd_ROE > 0, fwd_ROE, ROE))]

        # score_value 재계산 (forward 교체된 종목 포함, 전체 대형주)
        # 비금융 대형주
        non_fin_large <- result[is_large == TRUE & is_financial == FALSE]
        if (nrow(non_fin_large) > 0) {
          add_zscore(non_fin_large, "ROE",  "ROE_vs",  higher_is_better = TRUE)
          add_zscore(non_fin_large, "PER",  "PER_vs",  higher_is_better = FALSE)
          add_zscore(non_fin_large, "PBR",  "PBR_vs",  higher_is_better = FALSE)
          add_zscore(non_fin_large, "PCR",  "PCR_vs",  higher_is_better = FALSE)
          add_zscore(non_fin_large, "OPM",  "OPM_vs",  higher_is_better = TRUE)
          add_zscore(non_fin_large, "DEBT", "DEBT_vs", higher_is_better = FALSE)
          add_zscore(non_fin_large, "DIV",  "DIV_vs",  higher_is_better = TRUE)
          non_fin_large[, score_value := {
            roe_wt  <- fifelse(is.na(ROE_vs),  0, 0.15)
            per_wt  <- fifelse(is.na(PER_vs),  0, 0.20)
            pbr_wt  <- fifelse(is.na(PBR_vs),  0, 0.15)
            pcr_wt  <- fifelse(is.na(PCR_vs),  0, 0.10)
          ...
          # 대형주 결과에 반영
          result[is_large == TRUE & is_financial == FALSE,
                 score_value := non_fin_large$score_value[match(
                   result[is_large == TRUE & is_financial == FALSE]$종목코드,
                   non_fin_large$종목코드)]]
        }

        # 금융 대형주도 동일하게
        fin_large <- result[is_large == TRUE & is_financial == TRUE]
        if (nrow(fin_large) > 0) {
          add_zscore(fin_large, "ROE", "ROE_vs", group_col = NULL, higher_is_better = TRUE)
          add_zscore(fin_large, "PER", "PER_vs", group_col = NULL, higher_is_better = FALSE)
          add_zscore(fin_large, "PBR", "PBR_vs", group_col = NULL, higher_is_better = FALSE)
          add_zscore(fin_large, "DIV", "DIV_vs", group_col = NULL, higher_is_better = TRUE)
          fin_large[, score_value := {
            roe_wt <- fifelse(is.na(ROE_vs), 0, 0.30)
            per_wt <- fifelse(is.na(PER_vs), 0, 0.25)
            pbr_wt <- fifelse(is.na(PBR_vs), 0, 0.20)
            div_wt <- fifelse(is.na(DIV_vs), 0, 0.25)
          ...
          result[is_large == TRUE & is_financial == TRUE,
                 score_value := fin_large$score_value[match(
                   result[is_large == TRUE & is_financial == TRUE]$종목코드,
                   fin_large$종목코드)]]
        }

        # 별도재무 보정 재적용
        if ("holding_distortion" %in% names(result)) {
          result[holding_distortion == TRUE, score_value := pmax(score_value - 5, 0)]
          result[debt_override == TRUE, score_value := pmin(score_value + 1, 10)]
        }

        # score_best 재계산
        result[is_large == TRUE, score_best := {
          sb <- pmax(score_value, score_growth, na.rm = TRUE)
          fifelse(is.finite(sb), sb, NA_real_)
        }]

        log_step("Forward", "커버>=%d사 %d종목 PER/ROE forward 교체 → score_value 재계산",
                 FWD_MIN_BROKERS, n_fwd_eligible)
      }

      log_step("컨센서스", "완료: %d/%d 종목 확보", nrow(consensus_dt), length(large_codes))
    }
  } else {
    log_step("컨센서스", "fnguide_consensus.R 로드 실패, 스킵")
  }
} else {
  log_step("컨센서스", "FETCH_CONSENSUS=FALSE, 스킵")
}
...
out_cols <- c("종목코드", "종목명", "시장구분", "산업분류", "시가총액",
              ...
              "sep_ROE", "sep_DEBT", "holding_distortion", "debt_override", "consol_gap",
              "target_avg", "target_gap", "buy_ratio", "broker_count",
              "opinion_buy", "opinion_hold", "opinion_sell",
              "eps_fwd1", "eps_fwd2", "fwd_PER", "fwd_ROE",
              "blended_PER", "blended_eps")
